...
python -m content_hub.cli social get Q3_2026 --statuses "Awaiting Asset" --limit 5   # full cells
...
`social_edit_calendar` / `social_add_rows` have no `mock` (nothing is generated or spent, so `dry-run` is already the safe preview). `social_describe_calendar` and `social_get_rows` have no `mode` at all — they only read: `describe` gives the sheet's **shape** (columns + rows) so a caller composes edits against the real header row instead of guessing; `get_rows` gives the **content** (every cell value, filtered + paged) so a rewrite is an informed change rather than a blind overwrite.
...
content_hub/
  ...
  social/                ← workflow #1 (blog/ and email/ would be siblings)
    ...
    edit_ops.py          in-place cell edits + bulk row appends (schema-aware guardrails);
                         ...
                         get_rows() reads rows back in full (filtered + paged)
