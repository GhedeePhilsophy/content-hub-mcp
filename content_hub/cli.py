...
def _register_social(workflows) -> None:
    ...
    g.set_defaults(func=lambda a: social.generate_media(
        a.calendar_id, mode=a.mode, only=a.only,
    ...
    u.set_defaults(func=lambda a: sheet_ops.upload(
        a.calendar_id, a.version, quarter_folder=a.quarter_folder, replace=a.replace))
    ...
    d.set_defaults(func=lambda a: sheet_ops.download(
        a.calendar_id, dest_dir=a.out, quarter_folder=a.quarter_folder))
    ...
    p2.set_defaults(func=lambda a: preview.build_preview(
        a.calendar_id, a.version, out_path=a.out, quarter_folder=a.quarter_folder,
    ...
    sn.add_argument("--quarter-folder")
    sn.set_defaults(func=lambda a: sheet_ops.snapshot(
